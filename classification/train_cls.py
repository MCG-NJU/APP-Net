...
def backup_code(dst_path):
    ...
    for pattern in ['*.py', 'classification/config', 'classification/models', 'classification/data/*.py', 'pointnet2_ops_lib', 'classification/*.py']:
        for src in glob.glob(os.path.join(base_dir, pattern)):
            if 'build' not in src:
                dst = os.path.join(dst_path, 'backup')
                if 'Loader.py' in src or 'data_utils.py' in src:
...
@hydra.main("config/config_cls.yaml")
def main(cfg):
    ...
    checkpoint_callback = pl.callbacks.ModelCheckpoint(
        ...
        filepath=os.path.join(
            log_folder, "{epoch}-{val_loss:.2f}-{val_acc:.3f}"
        ),
        verbose=True,
    )
